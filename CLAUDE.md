...
3. **Safe VM sync** (when VM may be stale or you're unsure what's uncommitted):
   ...
   cd /home/user/projects/kaetram-agent
...
**Remote agent setup:**
- **GCP VM** (`vm.example.com`): Hosts Kaetram game server (:9001 WS) + client (:9000 HTTP). This is the game world.
...
- Agent on GPU VM connects browser to `http://vm.example.com:9000` and plays via Playwright.

### Remote access
| Machine | IP | SSH | Purpose |
|---------|------|------|---------|
| GCP VM (this) | vm.example.com | user@vm.example.com | Game server + client, data collection, training pipeline |
