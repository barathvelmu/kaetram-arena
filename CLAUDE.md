...
| `play_qwen.py` | Finetuned-model harness — Modal SGLang endpoint + MCP server. Multi-agent via `orchestrate.py --qwen` (peer to Claude/Codex/Gemini/OpenCode); solo dev invokes directly. |
...
- **Qwen bot username depends on the personality, not agent ID.** The in-house Qwen harness (`orchestrate.py --qwen`) names agents by personality so the in-game bot maps 1:1 to the personality variant being evaluated: `grinder` → `QwenGrinder`, `completionist` → `QwenCompletionist`, `explorer_tinkerer` → `QwenExplorer`. Logic lives in `orchestrate.Orchestrator.setup` (qwen_username_map) and is mirrored in `restart-single-agent.sh` + `restart-agent.sh` Mongo seeding.
