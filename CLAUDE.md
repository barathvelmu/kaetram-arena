...
**KTO pipeline validated, full run pending.** r6-KTO smoke test ran 10/10 steps cleanly (`train_loss=0.617`, KL active). The `ref_model=None + precompute_ref_log_probs=True` path is locked in — explicit reference-model KTO OOMs repeatedly on H100 80GB. r7-KTO will use the same config on top of r7 SFT merged weights once r7 finishes. Still awaiting the maintainer's greenlight for the full run.
...
|---------|------|------|---------|
| GCP VM (this) | 35.224.227.251 | user@35.224.227.251 | Game server + client, data collection, training pipeline |
