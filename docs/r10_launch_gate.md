...
## Lanes
- collaborator: `KAE-42` patches, remaining regression tests, decode-mode alignment, smoke SFT, eval matrix
- the maintainer: `KAE-41`, game/source knowledge, fresh Claude collection runs
...
- r10 dataset rebuilt: `17,002` train / `1,929` val (vs r9 `5,871` / `575`; +189%).
...
- [ ] `tests/test_tool_vocab_drift.py` exists and passes (training tools == MCP tools == `prompts/system.md` `<tools>` block).
...
- [ ] Dead tools (`accept_quest`, `clear_combat`, `talk_npc`) removed from `prompts/system.md` `<tools>` block + `dataset/qwen_sft/metadata.json` tools[].
...
## Out Of Scope
- `KAE-41` / upstream game-source fixes (the maintainer's lane)
