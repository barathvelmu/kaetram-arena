...
- Game client: `http://vm.example.com:9000`
- Game server WS: `ws://vm.example.com:9001`
- The agent opens Chromium to `http://vm.example.com:9000` and plays via Playwright MCP
...
```bash
# From the GCP VM (user@<GCP_IP>), SCP the prompts:
mkdir -p ~/kaetram-agent/prompts ~/kaetram-agent/logs ~/kaetram-agent/state
scp -r user@<GCP_VM_IP>:~/projects/kaetram-agent/prompts/ ~/kaetram-agent/prompts/
scp user@<GCP_VM_IP>:~/projects/kaetram-agent/state_extractor.js ~/kaetram-agent/
```
