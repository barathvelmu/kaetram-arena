...
\begin{tikzpicture}[
  ...
  \node[box,minimum width=2.6cm,fill=routeorange!10,align=center] (content) at (4.0,-0.65)
    {response body\\{\tiny(reasoning span + any trailing content)}};
  ...
  \draw[routered,dashed,rounded corners=3pt,thick]
    (2.45,-1.18) rectangle (11.55,1.18);
  \node[routered,fill=white,inner sep=1.5pt,anchor=south] at (7.0,1.15)
    {interface study: where recoverable calls are routed};
  \node[routeorange,align=center,anchor=north] at (4.0,-1.13)
    {all \SpanInsideReasoning{}/\SpanRecoveredCalls{} recovered calls\\were stranded inside the reasoning span};

  \node[font=\sffamily\bfseries\small,anchor=west] at (-0.3,-2.05)
    {(b) The audit advances only after each failure is localized};
  \node[stage] (s1) at (0.95,-3.05) {historical scores\\\textbf{confounded}};
  \node[stage] (s2) at (4.05,-3.05) {recovery test\\\textbf{\RecoveryEligibleGenerations{} / \RecoveryRawGenerations{} eligible}};
  \node[stage] (s3) at (7.15,-3.05) {V1 seed audit\\\textbf{\VOneDiverseSeedGroups{} / \VOneSeedGroups{} diverse}};
  \node[stage,fill=routeblue!8] (s4) at (10.25,-3.05) {two fixed panels\\\textbf{2,400/2,400 complete}};
  \node[stage,fill=routegreen!9] (s5) at (13.35,-3.05) {fresh route check\\\textbf{\RoutingVThreeFullPass{} / \RoutingVThreeProtocolValid{} pass}};
  \draw[line] (s1) -- (s2);
  \draw[line] (s2) -- (s3);
  \draw[line] (s3) -- (s4);
  \draw[line] (s4) -- (s5);
...
\begin{tikzpicture}[font=\sffamily\scriptsize]
  \begin{axis}[
    ...
    title={(a) Registered primary\\content-only recoverable calls},
