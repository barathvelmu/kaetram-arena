...
interface factors and exposed recoverable calls that the model wrote inside its
reasoning span instead of emitting through the structured field. Fourth, an audit discovered
...
which its routing rule declines to lift
...
\begin{enumerate}
  ...
  \item We localize where those calls sit and characterize the failure. All
  \SpanRecoveredCalls{} recovered calls lie in the endpoint's reasoning field,
  which its routing rule largely entails and which we therefore report as a
  mechanism disclosure rather than a finding. What the rows do add is the
  shape: every recovered row holds exactly one candidate with no reasoning text
  after it, so the model is not confusing channels but terminating one token
  before the call would have been routed.
...
unless an explicit recovery layer parses that block;
Section~\ref{sec:where-calls-sit} shows why, in this regime, adding such a
layer is the wrong repair. We use the following taxonomy:
...
\begin{description}
  ...
  V1 calls were well-formed canonical function blocks written inside the
  reasoning span rather than emitted through the structured field.
\end{description}
...
Two properties of the request path are load-bearing for every result below, and
neither is visible in the registered outcome. We state both because they
determine what that outcome can mean.

First, these studies were served by a local MLX backend launched with
thinking enabled, not by the non-thinking Modal configuration used for the
historical training corpus. The backend returns the model's reasoning and its
final message as \emph{separate} fields. It lifts a well-formed call into the
structured \texttt{tool\_calls} field only when that call appears in the final
message; a call appearing in the reasoning field is left where it is. The
studies are therefore off-parity with the training regime in this respect, and
we do not claim otherwise.

Second, the gateway in front of that backend reconstructs a single
\texttt{content} string by wrapping the reasoning in reasoning tags and
concatenating the final message. The tags in \texttt{content} are written by
the gateway, not emitted by the model; every released row is byte-consistent
with that construction. Any statistic computed over those tags describes the
gateway's formatting, so we report none.

The consequence for measurement is direct. The prospectively frozen parser
reads the reconstructed \texttt{content} string, so it reaches text the backend
had already classified as reasoning and had deliberately declined to route. A
claim about ``text'' in this setting must therefore say which field it means.
The registered outcome does not, so we make the relationship explicit in
Section~\ref{sec:where-calls-sit}.
...
unchanged & Mechanism disclosure largely entailed by the backend routing rule \\
...
interfaces change where in the response a syntactically valid call gets
written.

That pivot also changed the serving stack, and the resulting discrepancy is the
most important boundary in this paper. This factorial ran on the deployment's
non-thinking hosted configuration and produced no recoverable call in any of
the 664 generations that lacked a structured call. The interface studies ran on
a thinking-enabled local stack and produced recovered calls in roughly 29\% of
requests. The deployment lane sends the tool schema uniformly, so it
corresponds to the schema-present condition, which is the highest-rate
condition in the studies. The phenomenon the interface studies measure is
therefore not observed in the deployment lane at all, and the most economical
explanation is the serving configuration rather than the model: when the final
message carries the call, the backend routes it. We report the interface
results as an audit of a routing contract that can be configured into
existence, not as a measurement of this agent's deployed behaviour.
...
Given the routing rule in Section~\ref{sec:reasoning-regime}, the primary
outcome and the reasoning-field placement are very nearly the same event. The
backend lifts a well-formed call into the structured field exactly when it
appears in the final message. A recovered call is therefore almost by
construction a call that appeared in the reasoning field instead: a
counterexample would require a call the frozen parser accepts but the backend
does not. We report the localization because readers cannot infer it from the
outcome's name, not as an independent confirmation of it. Of the
\SpanRecoveredCalls{} recovered calls---\SpanVTwoRecovered{} in V2 and
\SpanVThreeRecovered{} in V3---all \SpanInsideReasoning{} sit in the reasoning
field, with \SpanRowsWithTrailingCall{} tool-call markers anywhere in the final
message across all \SpanTotalRows{} responses. The absence of counterexamples
is consistent with the routing rule rather than evidence beyond it.

What the rows do add is the shape of the failure, and it is narrower than a
channel-confusion story. Every one of the \SpanRecoveredCalls{} recovered rows
contains exactly one candidate, and none contains any reasoning text after that
candidate. The model reasons, emits a single well-formed call, and terminates.
It does not deliberate past the call, propose alternatives, or retract. So the
defect is not that the model conflated two channels; it is that generation
ended without the token that would have moved the call into the final message.

That distinction matters for the remedy, and it cuts against reading a
recovered call as a recovery \emph{opportunity}. A harness that promotes these
calls is overriding a routing decision the backend made deliberately, on the
basis of a field the model does not control, and it would do so uniformly
rather than case by case. We therefore do not recommend adding a recovery layer
here; the defect is upstream, in the generation-termination path, and the
registered label is retained only because it is frozen in the released
artifacts. We stop short of the stronger claim that promotion is unsafe because
the model might retract: these rows contain no retraction, so that argument has
no support here.

This localization is post-outcome and descriptive. It does not establish what a
different template, renderer, or termination rule would produce, or whether
promoting these calls would help or harm task performance.
...
absent from the structured field; it treats any nonempty structured field as
``structured.''
...
checkpoint-by-sample-index contrasts were positive. The VT component consists
entirely of calls written inside the reasoning span
(Section~\ref{sec:where-calls-sit}), so a rise in the pooled count is not a rise
in executable actions. A frozen strict-router replay would have preserved 373
structured rows and promoted 303 recoverable-text rows out of the reasoning
field, quarantining five and leaving 519 no-candidate rows unchanged. These are candidate actions, not evidence of
semantic appropriateness or utility. Because schema exposure also changed
routing, conditional route comparisons are selection-sensitive.
...
checkpoint-by-sample-index contrasts were positive. Strict replay would have
preserved 495 structured rows and promoted 352 recoverable-text rows out of the
reasoning span, quarantining 41 and leaving 312 without a candidate. These
counts strengthen the routing diagnosis across the second panel but inherit the
same post-hoc, selection-sensitive, no-utility boundary.
...
recovery-off control. These fixtures are author-authored and are not written
inside a reasoning span, so this diagnostic tests transport only; it does not
endorse promoting model-emitted calls out of that span.
Figure~\ref{fig:execution-check} summarizes the sequence.
In V2, all nine technical trials were protocol-valid,
...
All requests use a single thinking-enabled local serving stack, held constant
across checkpoints but off-parity with the non-thinking configuration under
which the historical corpus was rendered. That stack's routing rule is why
recovered calls sit in the reasoning field, so it bounds both the primary
outcome and the localization in Section~\ref{sec:where-calls-sit}. We ran no
non-thinking arm and no second stack, so we cannot separate the interface
contrast from the serving configuration, and we do not know whether a different
template or termination rule would relocate these calls. A prospective design
should cross the interface grid with the serving configuration and record the
chat-template arguments in the render attestation, which the present contract
does not.
