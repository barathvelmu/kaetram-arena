...
advances only after each failed check is localized. A recoverable call in
ordinary content is not visible to an executor that reads only the structured
field. Numbers are regenerated from the released public summaries.}
\label{fig:routing}
\end{figure}
...
and (c) show the registered primary outcome: a parser-recoverable call stranded
in ordinary content. Panel (b) is explicitly post-hoc and asks whether either
