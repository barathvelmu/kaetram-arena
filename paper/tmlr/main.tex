...
One-action execution check & 9/9 valid model-free technical trials &
Direct and recovered routing work for one fixture; strict disabled-arm
persistence equality does not \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{One-action local routing diagnostic}

To test the next boundary without a model call, we prospectively froze one
schema-valid \texttt{warp} candidate and routed it through structured input,
strict content recovery, and a recovery-off control. All nine trials passed the
frozen validity checks. Structured input and content recovery each passed all
three technical repeats: exactly one candidate was delivered, the application
predicate passed, and the immediate, delayed, and reconnect client projections
were in Mudwich.

\begin{table}[H]
\caption{Model-free one-action routing diagnostic. Counts are technical repeats
out of three, not independent samples. Recovery-off failed only the
preregistered exact database predicate.}
...
\toprule
Arm & Candidate routing & Client-state predicate & Full predicate \\
\midrule
Structured direct & Exactly one, 3/3 & Mudwich at all stages, 3/3 & 3/3 \\
Content recovery on & Exactly one, 3/3 & Mudwich at all stages, 3/3 & 3/3 \\
Content recovery off & Zero, 3/3 & Baseline at all stages, 3/3 &
0/3: database mismatch \\
\bottomrule
\end{tabular}
\end{table}

With recovery disabled, no candidate was invoked and all three client
projections remained at the registered baseline in every repeat. Nevertheless,
all three trials failed the full registered predicate because login and save
expanded sparse database records with default equipment, quest, achievement,
and skill entries plus session bookkeeping. We retain this failure rather than
redefining database equality after observing the result.

These technical repeats are not independent samples. The diagnostic shows that
one frozen action can traverse both active routes on this build; it does not
estimate a recovery benefit, validate model-selected calls, or establish quest
utility.
...
\paragraph{Client state and persistence equality are separate contracts.}
Recovery-off invoked no candidate and preserved every registered client
projection, yet login and save changed the database representation. This is not
evidence that the warp executed; it shows why client-visible state, candidate
delivery, and exact persistence equality must be reported separately.
...
diagnostic follows one author-fixed warp through MCP and client-state checks,
but it does not test naturally emitted V2 calls. Both active routes passed,
while recovery-off failed exact database equality because login and save
changed the persistence representation without invoking the candidate. With
one action, one start state, one build, and three technical repeats, this does
not establish safety, appropriateness, quest utility, or recovery benefit.
...
Local route diagnostic & All nine trials were valid. Both active routes passed
3/3; recovery-off invoked no candidate and preserved the client baseline 3/3,
but exact database baseline failed 3/3. This supports one-fixture route
operability and failure localization only. \\
...
The complete local-routing package contains a clean prelaunch receipt, runtime
identities, process-lifecycle evidence for every cold worker/MCP/browser
session, chained trial receipts, and a sealed analysis. These checks establish
author-attested internal consistency, not independent reproduction. Its
anonymous reviewer projection retains only the re-keyed scientific fields
needed to recompute the stage table; source and runtime authentication are
deferred until deanonymization.
...
The one-action executor diagnostic now covers parser acceptance, MCP
acceptance, and immediate, delayed, reconnect, and persistence projections for
one frozen candidate. The next study should use an outcome-unseen panel of
