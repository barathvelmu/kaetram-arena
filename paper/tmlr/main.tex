...
on both panels found more schema-valid candidates on both routes, but the
text-route candidates are exactly the ones an executor should not act on. Finally,
...
\begin{enumerate}
  ...
  responses carry any content outside that span. The failure is therefore
...
non-thinking configuration in which the generation prompt supplies a
closed-empty reasoning block. The served model nonetheless opens its own
reasoning span and closes it at the end of generation: in \SpanVTwoBalanced{}
of the \SpanVTwoRows{} V2 responses the returned \texttt{content} string begins
with an opening reasoning tag and contains exactly one opening and one closing
tag. This regime is held
constant across all three checkpoints and is identical to the one under which
the historical training data was rendered, which is why it was chosen; it is
not a defect introduced for this study.

So the \texttt{content} field is not user-facing prose: it is a reasoning span,
optionally followed by post-reasoning content. The endpoint also exposes that
same reasoning text in a separate \texttt{reasoning} field without removing it
from \texttt{content}, and the prospectively frozen parser reads the whole
\texttt{content} string. It therefore reaches text the endpoint had already
classified as reasoning. Any claim about ``text'' here must say which part of
the response body it means; the registered outcome does not, so we measure it
in Section~\ref{sec:where-calls-sit} rather than assume it.
...
\SpanVThreeRecovered{} in V3---all \SpanInsideReasoning{} were written before
the reasoning span closed, and in every case the endpoint's own parse placed
the call text in the \texttt{reasoning} field rather than in post-reasoning
content. Across
all \SpanTotalRows{} responses in both panels, only
\SpanRowsWithTrailingText{} carry any content outside the reasoning span as the
endpoint parsed it, and \SpanRowsWithTrailingCall{} carry a tool-call marker
there. The split holds under both interface conditions, so it is a property of
the serving regime rather than of the treatment.
...
structured rows and promoted 303 recoverable-text rows out of that span---the
operation we recommend against---while quarantining five and leaving 519
no-candidate rows unchanged. These are candidate actions, not evidence of
