...
The authors verified the evidence, analyses, citations, and final claims and
retain full responsibility for the work.}}
...
\begin{abstract}
A language-model agent can produce a tool-call candidate that never becomes an
environment action because it surfaces outside the field consumed by the
executor. We audit this hidden routing step in a 2B agent controlling a
persistent game through 17 typed tools. Historical scores confounded weights,
states, interfaces, and runtime repair, while a prospective 18-cell recovery
test never engaged its treatment in 958 generations. We therefore studied raw
response routes on fixed decision states.

Two thinking-enabled panels found that native schema exposure increased
body-only parser-candidate incidence at every checkpoint: +23, +13, and +10
percentage points on a registered outcome-unseen panel, and +13, +5.5, and +9
points on a same-project robustness panel. Because those studies were
off-parity with the non-thinking deployment path, we preregistered a matched
bridge. Across 1,020 new pilot-unseen requests, thinking-off produced zero
body-only candidates at all three checkpoints (0/1,020) and 905/1,020
structured calls. Its registered body-only contrasts versus retained
thinking-on were $-28.2$, $-22.1$, and $-28.8$ percentage points. Of 269
matched thinking-on body-only cells, 248 (92.2\%) aligned with a structured
thinking-off call and 21 with no candidate. The zero agrees with the unmatched
non-thinking deployment observation (0/664 structureless generations), though
stack differences prevent attributing it to thinking mode alone.

All 701 body-only candidates across the earlier panels were reconstructed from
the reasoning field, which is largely implied by the routing implementation;
only 656 passed the frozen tool schema. The contribution is a
reviewer-recomputable audit documenting large routing contrasts associated with
model-visible schemas and thinking-mode rendering on these finite local grids.
It does not rank checkpoints, establish a recovery or training benefit,
validate action appropriateness, or measure gameplay utility.
\end{abstract}
...
upgrade the first result retrospectively. Two thinking-enabled panels showed
the same-direction schema--routing pattern on two same-project retained panels.
Finally, after discovering that this serving mode differed from deployment, we
preregistered a matched
...
field. Every body-only candidate in the V2/V3 panels was reconstructed from the
endpoint's reasoning field, outside the executor-consumed structured route
...
\begin{enumerate}
  ...
  contrast across three fixed checkpoints and 1,020 new requests, with
...
\subsection{How a parser candidate can disappear from execution}
...
\texttt{enable\_thinking=true}. The setting was explicit in the study launch
code, but it was off-parity with the non-thinking historical deployment path and with
...
All 1,020 new requests completed without a retry or failure, and each endpoint
identity remained stable. The registered criterion passed: thinking-off minus
thinking-on body-only incidence was $-28.2$, $-22.1$, and $-28.8$ percentage
points at Base, round 2, and round 3. The corresponding structured-call
contrasts were $+58.8$, $+58.8$, and $+60.0$ points. In paired classification,
the 269 thinking-on body-only cells aligned with 248 structured and 21
no-candidate thinking-off responses. Figure~\ref{fig:thinking-parity}
shows the full mutually exclusive partition rather than only the registered
outcome.

\begin{figure}[t]
\centering
\ServingRegimeParityFigure
\caption{Matched comparison of the registered thinking-mode render contrast on
the fixed local pipeline. Panel (a) shows mutually exclusive response routes for
the same \ParityRequestsPerCheckpoint{} request cells at each checkpoint: a
structured call, a parser-recognized call found only in the response body, or
no parseable call. Each pair compares the retained thinking-on arm with the
prospectively registered thinking-off arm; stacks show exact counts. Panel (b)
shows thinking-off minus thinking-on changes for the two call-bearing routes.
The registered criterion---a negative body-only difference at all three
checkpoints---\ParityDirectionalVerdict{}. The first
\ParityExcludedPilotStates{} pilot states are excluded. The separately
recreated arms match the registered checkpoint, tokenizer, chat-template file,
sampling, and snapshot identities. This finite-grid result does not prove
literal single-factor isolation, isolate provider differences, or measure
gameplay utility.}
\label{fig:thinking-parity}
\end{figure}
...
\subsection{Where body-only parser candidates actually sit}
...
Each body-only row contained one parser candidate and no later reasoning text.
...
Invalid structured-envelope counts were lower with native schema in V2: 34 to
1, 24 to 6, and 18 to 3 per 200 requests at Base, round 2, and round 3.
...
channels and measures how a thinking-mode render contrast changes candidate
routing within one checkpoint family.
...
A tool call is not yet an action: it must surface in a field the next component
reads, pass the tool schema, reach the executor, and produce an appropriate
state change. On our matched fixed local panel, thinking-off minus thinking-on
contrasts were $-22.1$ to $-28.8$ percentage points for body-only
parser-candidate incidence and $+58.8$ to $+60.0$ points for structured-call
incidence. In paired classification, the 269 thinking-on body-only cells
aligned predominantly with structured thinking-off responses.

The earlier schema result is now properly conditional. Under the
thinking-enabled renderer, native schema exposure increased body-only incidence
at every checkpoint on a registered outcome-unseen panel and a same-project
robustness panel. Localization of all 701 candidates to the reconstructed
reasoning field explains the measurement path but is largely entailed by the
routing implementation; it does not reveal model intent or the correct repair.

These findings do not rank checkpoints, identify a training or recovery
benefit, validate the appropriateness of naturally emitted candidates, or
establish gameplay utility. They show why agent evaluation must bind the render
contract and retain each
response field instead of collapsing model generation, routing, parsing, and
execution into one score. Before attributing a long-horizon result to model
weights, record where each proposed action or candidate stopped.
...
engaged; no recovery effect is identified. The unmatched deployment stack had
0/958 body-only candidates overall and 0/664 among structureless generations;
V2 and V3 had 701/2,400 overall and 701/1,532 among structureless generations. \\
Thinking-mode bridge & The registered matched panel completed 1,020 new
requests. Thinking-off minus thinking-on contrasts were $-28.2$, $-22.1$, and
$-28.8$ points for body-only incidence and $+58.8$, $+58.8$, and $+60.0$ points
for structured incidence. The
arms use separately recreated pinned environments, so literal single-factor
isolation and provider parity are not claimed. \\
...
Reasoning-field placement & All \SpanRecoveredCalls{} body-only parser candidates lay in
...
holds one parser candidate and no later reasoning text; only 656 passed the
tool schema. Finish reasons were not retained, so termination cause, intent,
and remedy are unidentified. \\
...
separation. Exact body-only-candidate details use the shared frozen canonicalization
code.

The serving-regime parity bundle binds all 1,020 new thinking-off response
rows, prelaunch and postflight receipts, three run indexes, the analysis index,
and the preregistration hash. Its fail-closed verifier reclassifies the raw
responses, authenticates the retained V2 thinking-on artifact before joining
it, checks all paired cell identities, and recomputes every checkpoint contrast
and the registered all-three-checkpoint verdict. The anonymous projection keeps
both arms' semantic response fields and finite-grid labels so the same route
counts and transitions can be recomputed without exposing source-history or
...
The matched bridge shows a large route contrast between retained thinking-on
and new thinking-off arms on the fixed MLX grid. A tighter mechanism study
should recreate both arms concurrently from one source/runtime receipt, retain
backend finish reasons, and repeat the contrast on the historical deployment
backend. A separately registered stop-rule arm that closes the reasoning span
before generation continues would test termination mechanics.
...
model-emitted structured calls, measure downstream utility, and preregister separate
