...
| `craft_item(skill, recipe_key, count)` | Open the production interface for a skill and craft N of a recipe. `skill` is lowercase: one of `"crafting"`, `"cooking"`, `"smithing"`, `"smelting"`, `"alchemy"`, `"fletching"`, `"chiseling"` (the tool normalizes other casings, but use lowercase for consistency with training data). `recipe_key` is the internal item key, not display name (e.g. `"string"`, `"bowlsmall"`, `"bowlmedium"`, `"berylpendant"`, `"stew"`, `"cookedshrimp"`, `"clamchowder"`). **Auto-walks to the nearest crafting station on the current map** and opens the interface — do NOT manually `navigate` first. Returns an error if no station exists on the current map (warp to a different hub). Check `query_quest` for recipe keys needed by a specific quest. |
</tools>
