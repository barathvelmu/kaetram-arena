...
| `gather(resource_name)` | Gather from nearest matching resource by exact display name (e.g. `"Oak Tree"`, `"Coal Rock"`, `"Blue Lily Bush"`, `"Shrimp Fishing Spot"`, `"Clam Spot"`). Full resource list + skill gates in `game_knowledge` RESOURCE LOCATIONS. Walks to it, harvests one cycle, reports items gained. When `items_gained` is `"none"`, the response also includes a structured `gate` block (`{skill, required_level, current_level, gated}`) and a human-readable `why_no_items` — trust those: if `gate.gated` is `true`, you cannot harvest this resource yet. Either grind the named skill or pick a different resource/quest. |
...
9. **SEEK QUEST** — No active unfinished quest → pick the next unfinished quest from `game_knowledge` PRIMARY OBJECTIVE. **Core 5 are the benchmark — prefer them when reachable.** If every unfinished Core 5 is gated (`live_gate_status.gated: true`) or unreachable (BFS no-path even after warp + multi-leg), take an EXTRA or bonus quest in the meantime — the XP, items, achievements, and warp unlocks often clear Core 5 prereqs. The Off-limits table is informational — don't accept those quests (their rewards/items are broken). If the row has a gate, prereq, or multi-step chain, `query_quest(exact_quest_name)` before travel. Buy shop items first if the quest requires them.
10. **ACCEPT** — Quest NPC nearby (`quest_npc: true`, distance ≤ 10) AND you've decided this is the next quest to start → **first** call `query_quest(exact_quest_name)` and inspect `live_gate_status`. If `live_gate_status.gated` is `true`, **do NOT accept this quest** — its blockers (skill level, prereq quest, achievement) are not yet satisfied; pick the next unfinished Core 5 instead. Only when `live_gate_status.gated` is `false` (or `query_quest` returned no live status) do you call `interact_npc(npc_name, accept_quest_offer=True)`. If you only want to read the dialogue first (e.g. to confirm what they offer), call `interact_npc(npc_name)` without the flag.
