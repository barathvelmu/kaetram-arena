...
player states whose legal reachability was not certified. Evaluated from a fresh
canonical-start world with no intermediate-state initialization, the resulting policy scores 15/30; all three clustered prompt
...
session. These observations motivate a confirmatory comparison in which
...
\begin{enumerate}
  ...
  \item We document an interface failure that token-average analyses obscure: in
  a targeted teacher-forcing probe, malformed history reverses the teacher's
  local preference. This secondary diagnostic is separated from a runtime
...
fresh canonical-start world with no intermediate-state initialization and with
recovery disabled. Its historical gameplay-RNG seed was not preserved. It scores 15/30, with each prompt
...
interface. The reported recovery-off contrast is base 12/30 versus round 2
...
round was followed by canonical-start passage of a previously unseen wall. That sequence
