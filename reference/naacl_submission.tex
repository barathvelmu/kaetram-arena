...
\title{Targeted Failure-State Initialization for On-Policy Distillation:\\
An Audited Case Study in a Persistent Tool-Use Agent}
...
\begin{abstract}
On-policy distillation supervises only states visited by the student. This is
restrictive for persistent agents: early failures can prevent them from reaching
states at which a teacher is more capable. We study a 2B-parameter language model
...
student rollouts with verified intermediate world states. Evaluated from a fresh,
...
session. These observations motivate a confirmatory comparison against natural
OPD and a success-prefix temporal curriculum, with model weights and interface
recovery isolated. We present an audited case study and falsifiable protocol,
not established cross-task generality.
\end{abstract}
...
on-policy distillation to the student's own emitted tokens and compare natural
student visitation with a mixture that begins some rollouts from verified
intermediate database states.
...
\begin{enumerate}
  \item We instantiate targeted failure-state reset as a control point for
  on-policy distillation and document an exploratory case in which a seeded
  training round is followed by unseeded passage of a previously unseen wall.
...
\citep{wang2026tcod}. Our narrower target is a verified failure state selected
for high student-visitation deficit and teacher competence, evaluated against
both natural OPD and a TCOD-style success-prefix baseline.
...
\paragraph{Visitation ablation.}
Train two fresh students from the same checkpoint with every setting fixed:
natural student visitation only versus the same natural data budget mixed with
verified milestone states. Match teacher, optimization, action-token budget,
environment interactions, and training-seed schedule; evaluate both unseeded.
...
Hold out at least one quest from state seeding and compare full walkthrough
against no-walkthrough prompts. A second environment or model family is needed
for any broad claim. Plain natural-visitation OPD, corrected-interface SFT, and
a matched TCOD-style success-prefix curriculum are minimum baselines under matched interaction and
teacher-scoring budgets.
...
starts is therefore preempted. DAgger formalizes iterative data collection under the learner's
state distribution \citep{ross2011dagger}. Reset-along-demonstration, Backplay,
...
\citep{wang2023voyager,zheng2025agenttrek}. Our narrower question is whether
state visitation controls which teacher capabilities are available to a small
student under dense grading.
...
The observed case supports a concrete hypothesis: for persistent tool-using
agents, controlling where the student acts may matter as much as optimizing how
it matches a teacher. Natural-visitation OPD changed behavior without changing
score; a later state-augmented training round was followed by unseeded passage
of a previously unseen wall. A malformed-prefix probe further shows why
teacher-forced token feedback can fail at the tool boundary. Replication,
isolation, and held-out transfer are the next scientific results, not boxes that
prose can check.
...
systems without restorable state. The failure state was selected
retrospectively, so selection bias is possible. Teacher competence at the target
state is observed in a small number of trajectories, not estimated precisely.
...
and full dependency installation has not been demonstrated from the repository
root. These omissions are release blockers, not appendix footnotes.
