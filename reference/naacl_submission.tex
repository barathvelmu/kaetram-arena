...
Let $x\in\mathcal{X}$ denote the persistent external world state and
$h\in\mathcal{H}$ the model-visible interaction history. The policy state is
...
$\mu$ initialize external states, $q(h\mid x)$ initialize visible histories,
and $d^{\pi}_{\mu,q}(x,h)$ denote the joint occupancy induced thereafter by
policy $\pi$ and the environment dynamics. The canonical serving initializers
...
&=\mathbb{E}_{(x,h)\sim d^{\pi_S}_{\mu,q}}
  \mathbb{E}_{a\sim\pi_S(\cdot\mid x,h)} \\
&\quad\left[\log\pi_S(a\mid x,h)-\log\pi_T(a\mid x,h)\right].
\end{aligned}
\label{eq:opd}
\end{equation}
...
can also change the compatible $h$, the confirmatory design crosses external-
state initialization with matched history construction rather than treating a
...
witness trajectory from the canonical start or an invariant certificate whose
checker establishes a valid path to the snapshot. Evaluation is always
...
constructor $q$. The confirmatory selector admits an external state only if it
passes world invariants, is unfinished and task-relevant, is recoverable,
...
The repository contains prospective components for parts of the following
design, but not yet an integrated matched training launcher. Results are not
reported until the full protocol and immutable run bundles exist.
...
The six primary training arms are natural OPD, targeted external states,
random-valid external states, progress-matched external states, TCOD-B2F
...
\paragraph{External-state by history ablation.}
For the same certified external state, compare (i) a direct snapshot with the
...
trajectory. This distinguishes access to $x$ from information or error patterns
...
The audited case supports a concrete hypothesis: low-occupancy external states
...
The prospective launcher can seed inference, but the game still uses unseeded
environment randomness and therefore fails closed for confirmatory launch.
These omissions are release blockers, not appendix footnotes.
