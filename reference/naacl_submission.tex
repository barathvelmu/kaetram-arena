...
second stage, a faithful Guided-OPD objective, or accelerator execution. Results
are not reported until those remaining implementation boundaries, the full
protocol, and immutable run bundles exist.
...
short-horizon reward stage. A third draft corrects Guided-OPD to complete-turn
teacher/student sampling, freezes its cosine training-step curriculum, and
validates mixed-history role traces, but deliberately blocks the legacy trainer.
Verified training bundles, a live Guided-OPD mixed-turn collector and
actor-conditional forward/reverse-KL objective, the SCoRe-style second-stage
artifacts, and accelerator execution remain absent. These
