...
- **r10 dataset rebuilt (May 7–8).** `dataset/qwen_sft/` regenerated from the post-Core-3 Claude corpus only: 5 runs × 3 agents = 135 sessions, 19,152 raw turns. Mixed-mode thinking ratio gate (≤25% non-thinking assistant turns, `6601b3c` May 8) applied per Qwen3.5 Thinking Mode Fusion guidance → **14,346 train / 1,398 val = 15,744 records**. Provenance in `metadata.json`. Old r10 + backups in `dataset/_archive/`. Launch-gate concept retired — benchmark = live Core 3 completion in `tests/e2e/quests/`.
...
- **r10 training ETA (revised May 8).** Thinking-ratio gate reduced corpus to 15,744 records, dropping ETA from ~62h to ~38h on H100. Still exceeds Modal's 24h cap — checkpoint-resume or packing decision pending.
...
- **r10 SFT launch:** Dataset rebuilt 2026-05-08 (15,744 records, post-thinking-ratio-gate). Training blocked on ETA decision (~38h vs Modal 24h cap — checkpoint-resume or packing). Once resolved, run launches `kaetram-qwen3.5-9b-r10` on Modal H100; eval matrix compares r10-sft vs r9-sft vs base on Core 3 via N-model Bonferroni pipeline.
