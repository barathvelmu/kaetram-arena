...
## Recent Major Changes (Apr 24 – May 8, 2026)
...
- **r10 dataset rebuilt (May 7–9).** `dataset/qwen_sft/` regenerated from the post-Core-3 Claude corpus only: 5 runs × 3 agents = 135 sessions, 19,152 raw turns. Mixed-mode thinking ratio gate (≤25% non-thinking assistant turns, `6601b3c` May 8) + strict 16,384-token truncation gate (commit `1175358` May 9, dropped 4,906 overlong records) → **8,405 train / 851 val = 9,256 records** (live counts in `metadata.json`). Provenance in `metadata.json`. Old r10 + backups in `dataset/_archive/`. Launch-gate concept retired — benchmark = live Core 3 completion in `tests/e2e/quests/`.
...
- **r10 training ETA (revised May 9).** Truncation gate reduced corpus from 14,162 checked to 9,256 kept records, dropping ETA from ~38h to ~22h on H100. Now fits within Modal's 24h timeout — packing/checkpoint-resume decision no longer blocking.
...
- **r10 SFT launch:** Dataset rebuilt 2026-05-09 (9,256 records, post-thinking-ratio + truncation gates). ETA ~22h on H100 — fits Modal's 24h timeout. Run launches `kaetram-qwen3.5-9b-r10` on Modal H100; eval matrix compares r10-sft vs r9-sft vs base on Core 3 via N-model Bonferroni pipeline.
