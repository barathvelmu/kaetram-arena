...
- **r10 SFT eval — COMPLETE (May 19–22).** Trained on Modal H100 ~22h; deployed via `serve_modal.py` (`min_containers=0`). Eval matrix n=4 base / n=3 SFT, all 3h+, clean wire after `play_qwen.py` JSON-dict fix `7bf7c8d`. Base 7/30 every run, SFT mean 2.0/30 → 3.5× regression (Mann-Whitney exact p=0.029). Headline result + mechanism + r11 plan in `experiments/r10-discussion.md`; paper-ready framing in `paper/contribution.md`.
