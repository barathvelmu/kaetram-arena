...
- **Data scale milestone (May 3, updated May 12).** Post-archive-split active corpus: 30 runs / 1,611 sessions across 3 agents (post-Core-3 Claude only). 1,694 non-active sessions archived under `dataset/raw/_archive/` (1,049 Claude + 645 non-Claude). r10 dataset uses 5 source runs / 135 sessions from this active pool. Rick's Roll stage-2+ prompt knowledge shipped May 1 (`154badc`).
