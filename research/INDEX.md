...
- **r10 dataset rebuilt (May 7).** `dataset/qwen_sft/` regenerated from the post-Core-3 Claude corpus only: 5 runs × 3 agents = 135 sessions, 19,152 raw turns → **23,225 train / 2,264 val = 25,489 records** (rebuilt May 7 after pipeline cleanup `ad66cca` + dead-code removal `09e611d`). Provenance in `metadata.json`. Old r10 + backups in `dataset/_archive/`. Launch-gate concept retired — benchmark = live Core 3 completion in `tests/e2e/quests/`.
...
- **r10 training crisis (May 7).** 25,489 records at `MAX_SEQ_LEN=16384` without packing → ~62h ETA on H100, exceeding Modal's 24h cap. Decision pending on packing vs sequence-length filtering.
...
- **r10 SFT launch:** Dataset rebuilt 2026-05-07 (25,489 records). Training blocked on packing/ETA decision (~62h vs Modal 24h cap). Once resolved, run launches `kaetram-qwen3.5-9b-r10` on Modal H100; eval matrix compares r10-sft vs r9-sft vs base on Core 3 via N-model Bonferroni pipeline.
