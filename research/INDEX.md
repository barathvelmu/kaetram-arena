...
- **r10-SFT vs base headline result (May 19–20).** Clean comparison (n=3 base / n=2 SFT) after `play_qwen.py` JSON-dict fix landed in `7bf7c8d`. Base (n=3 runs, identical reproduction): `(grinder=1, completionist=3✅, explorer=3✅) = 7/30 Core 3 stages` every single time. SFT (n=2 clean runs): `run_20260520_014319 = 3/30`, `run_20260520_044433 = 1/30`, mean **2.0/30**. **3.5× drop on Core 3 stages.** Foresting completion rate: base 67% (6 of 9 agent-attempts), SFT 17% (1 of 6) — **4× drop**. Mechanism: completionist `interact_npc` calls collapsed 6.25× (62.5 → 10) and `query_quest` collapsed 5.0× (68.5 → 13.5), while `navigate` was *amplified* 4.08× (32 → 130.5). Tool-mix tracks training corpus distribution (corpus interact_npc=3.6%, navigate=21.6%). This is the **corpus-prior-becomes-inference-prior** result — catastrophic capability suppression on the verbs the corpus under-represented. Buggy May 12 SFT run (`run_20260512_120516`, ran on pre-fix code) deleted from corpus. Full data in `research/experiments/r10-concerns.md`.
