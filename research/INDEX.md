...
- **Loss masking fix (KAE-25):** `completion_only_loss=True` with `text` field dataset does nothing — TRL only masks for `prompt`+`completion` format. Need `assistant_only_loss=True` with raw `messages` for r8. collaborator handling.
