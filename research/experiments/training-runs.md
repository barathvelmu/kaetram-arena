...
| r10 | May 7-10 | SFT (dataset) | 8,510 train / 853 val (9,363 total) | Claude corpus only, 3 agents (grinder / completionist / explorer_tinkerer). Mixed-mode thinking-ratio gate (≤25% no-think) + strict-16,384 truncation gate (dropped 4,799 overlong). Reasoning rendered verbatim; `_drop_overlong` is the only length authority. | Dataset rebuilt 2026-05-10. ETA ~22h on H100 (fits Modal 24h). |
...
**Data scale (May 3, updated May 14):** Active corpus (post-archive-split): 34 runs / 2,018 sessions across 3 agents (agent_0: 14/571, agent_1: 10/728, agent_2: 10/719). Includes both Claude collection and Qwen eval runs. 1,694 sessions archived. Rick's Roll stage-2+ knowledge **now in `game_knowledge.md`** (shipped May 1 commit `154badc` — puzzle-room door chain, Lena coords, all 7 decoy ladders, 2-call turn-in caveat).
