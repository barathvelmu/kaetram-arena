...
| Reachability-targeted external-state initialization improves OPD | Protocol and prospective selection/seeding code only; the historical milestone was hand-selected | Unsupported outcome; testable method | “We propose a frozen selector and matched control suite.” | Candidate artifacts, matched natural/random/progress/TCOD/Guided arms, independent unseeded runs, and run-level uncertainty |
| Round two is a pure weights improvement | R2 evaluation reportedly used recovery off, but exact historical schema, prompt, checkpoint, seed, and environment parity cannot be reconstructed | Least-confounded historical weights-only observation | “Round two without recovery was reported at 15/30 versus the base run's 12/30; exact parity and independent reproduction remain unavailable.” | Replicate under frozen manifests and the canonical render contract |
...
- The dedicated r10 eval servers used `kaetram_eval` from April 25 onward while the wrapper reset and harness snapshots targeted `kaetram_devlopment`. Those r10 arms are quarantined pending raw proof of clean starts or reruns. The June OPD Core-3 sequence used the separate orchestrator lane, whose server and reset both target `kaetram_devlopment`, so PR #37's specific mismatch does not directly apply to those run IDs.
