...
| Environment-state seeding transfers competence | Base/r1 0/3 and r2 3/3 on the Herbalist wall; r2 15/30 versus base 12/30; eval is unseeded | Strongest observational sequence; causal test pending | “The seeded-training round was followed by unanimous unseeded wall passage in one three-agent run.” | Otherwise identical seeded versus unseeded training arms and independent run-level replication |
| Reachability-targeted external-state initialization improves OPD | Protocol and prospective selection/seeding code only; the historical milestone was hand-selected | Unsupported outcome; testable hypothesis | “We specify a frozen selector and matched control suite.” | Witness-certified candidate artifacts, matched natural/random/progress/TCOD/Guided arms, state-by-history ablations, independent unseeded runs, and run-level uncertainty |
...
| Teacher-forcing copy prior | Two-sided probe reports roughly 85% teacher probability on continuing malformed syntax in context despite clean teacher generation | Post-hoc secondary diagnostic, not an established mechanism | “A targeted probe reveals a context-dependent endorsement reversal.” | Larger causal sample, clean/malformed paired histories, multiple defects/models, confidence intervals |
| Harness recovery cures the format defect | Rewritten logs contain 405 recovery markers and no repeated marker within a session | Logged proxy only | “No repeat recovery marker appears after the correction in the rewritten session logs.” | Preserve raw pre-rewrite emissions and replicate under frozen logging |
...
- Confirmatory inference requests can now carry deterministic per-replicate seeds, but the Kaetram game still uses unseeded `Math.random()`. The prospective launcher correctly fails closed until environment RNG seeding is implemented and recorded.
