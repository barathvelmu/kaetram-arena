...
**Statistical power.** The current prospective contract schedules 20 run-level replicate clusters, but its 80% calculation is explicitly conditional on a three-stage minimum relevant paired difference and paired-difference SD no greater than three stages after multiplicity adjustment across seven estimands. That variance bound has no independent pilot support yet. Before compute, justify it with independent pilot data or a conservative prespecified variance grid (or use blinded variance re-estimation with a fixed maximum); neither 20 nor the historical default of 50 is automatically confirmatory.
